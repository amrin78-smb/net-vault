...
param(
  ...
  [int]$WsPort = 3010,
  # SHA-256 fingerprint of the app server's agent-WebSocket certificate. Those
  # certs are self-signed and generated by the installer, so there is no chain to
  # validate and the agent verifies the fingerprint instead. Optional: without it
  # the channel is still encrypted, just unauthenticated, and the agent says so.
  # Paste openssl's output as-is - colons and case are ignored.
  [string]$WsFingerprint,
  # Only needed for a SPLIT deployment where DDIVault runs on its own host and
  # therefore presents a different certificate. On the usual single-host install
  # one cert serves both listeners and -WsFingerprint covers them both.
...
$cfg = [ordered]@{ serverUrl = $ServerUrl; apiKey = $ApiKey; wsPort = $WsPort }
# Per-app pins: SpanVault and DDIVault present their own certificate when they
# are on different hosts, so a single shared pin would verify one and reject the
# other. core/transport.js reads config.wsFingerprints[<app>].
...
# -Depth 4: wsFingerprints nests one level deeper than the flat shape above, and
# the default of 2 would serialise it as "System.Collections.Hashtable".
