...
function Start-EngineWorker([string]$innerCmd) {
    ...
    # Signal success via a STATUS FILE, not the exit code: Start-Process -PassThru (no
    # -Wait, redirected output) returns a NULL ExitCode, so it can't be trusted. Engine
    # completes -> 'OK'; a thrown failure -> 'FAIL'.
  ...
  <Grid>
    ...
    <Grid Grid.Row="1" Margin="20,16,20,8">
      ...
      <StackPanel x:Name="ConfigPanel">
        ...
        <TextBlock TextWrapping="Wrap" Foreground="#64748B" FontSize="12" Margin="0,0,0,4"
          Text="Database credentials are generated automatically and are unique to this machine - there are no passwords to set."/>
...
# ---------- grab named elements ----------
$el = @{}
'ConfigPanel','TxtInstallDir','TxtServerIP','ChkLog','ChkDdi','ChkSpan',
...
$el.BtnInstall.Add_Click({
    ...
    # DB credentials are auto-generated by the installer (stored in C:\ProgramData\NocVault) - no password args.
    ...
    $script:timer.Add_Tick({
        ...
        if ($script:proc.HasExited) {
            ...
            if ($status -eq 'OK') {
                ...
                Finish $false "Install did not complete - see the FATAL line / saved log."
            }
        }
    })
    $script:timer.Start()
