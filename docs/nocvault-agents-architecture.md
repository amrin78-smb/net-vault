...
reconciliation + signed self-update, **shipped**) · **`nocvault-agents-phase4-plan.md`** (DDIVault module +
slim SpanVault; splits into 4a slim-SpanVault+hub-lifecycle and 4b the large DDIVault edge module).)*
...
| Signed **bundle** verification | **Shipped in Phase 3** (netvault 1.27.0 / agent 2.3.0): the hub signs a per-file sha256 manifest (Ed25519, `scripts/sign-agent.js`); the agent verifies the signature BEFORE any download (`core/updater.js`'s `verifyManifest()`, the security gate), re-hashes every downloaded file against the manifest, and applies via a crash-safe staged swap with automatic rollback on a crash-looping build (`core/apply-update.js`). | Was previously unsigned-over-trusted-LAN (acceptable short-term, fleet-wide RCE risk over WAN) — closed before wide rollout, per the original leaning below. |
