...
(c) CornersToggle — Rounded/Square segmented control, rendered in app/(app)/layout.tsx beside ThemeToggle. Deliberately NOT on /settings: that page redirects non-admins to /dashboard, and this is a per-browser preference every role must be able to set. Reads its value in useEffect, never at render (the <html> attribute does not exist during SSR — reading at render is a hydration mismatch).
