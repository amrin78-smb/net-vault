...
## app/components/
Real, actively-imported second location — not dead code: UpdateNotifier is imported by app/(app)/layout.tsx (the main app shell) and rendered in every app page. It is a suite-standard top-bar banner (update-available notice), not route-colocated glue for a single route. Only 1 file lives here; no other components have been added to this location — treat it as a one-off exception to the components/ convention rather than a parallel pattern to keep extending.
