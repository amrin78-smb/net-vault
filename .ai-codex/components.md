...
- (c) `MfaCard` (no props) — `app/(app)/settings/MfaCard.tsx`, rendered by Settings → Security. Self-service TOTP enrolment: setup → verify a code → enable + show backup codes once. Available to EVERY role (two-factor is not an admin privilege), and hides the disable control when policy requires MFA for the user's role.
