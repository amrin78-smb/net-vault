...
(c) UpdateFailureBanner  (none) — polls /api/system/last-update-status every 5min, shows a dismissible red banner if the last Update-NetVault.ps1 run failed (stage/error code/rolled-back state) or an amber one for a non-fatal schema-apply warning; rendered only for admin/super_admin (layout.tsx and launcher/page.tsx each gate it on session role), default export [app/components/UpdateFailureBanner.tsx]
